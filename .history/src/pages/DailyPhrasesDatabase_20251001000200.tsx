// Daily Phrases Database — improved transliteration + basic harakah overrides
// Drop-in replacement. Keeps your API and helpers the same.
...
// -----------------------------------------------------------------------------
// Transliteration helpers (context-aware, not just letter-by-letter)
// -----------------------------------------------------------------------------
const AR_DIAC = /[ًٌٍَُِّْـ]/g; // strip diacritics if any
const SUN_LETTERS = new Set(['ت','ث','د','ذ','ر','ز','س','ش','ص','ض','ط','ظ','ل','ن']);

// Per-token overrides (common function words)
const TOKEN_OVERRIDES: Record<string, { harakah?: string; translit?: string }> = {
  ...
  'ال': { harakah: 'ال', translit: 'al' },
};

// Phrase-level overrides (exact match, incl. punctuation)
...
// Base character map (used inside a token with small heuristics)
...
function transliterateToken(token: string): string {
  ...
  // Definite article handling: keep it readable as "al-..." (no sun-letter doubling here)
  if (raw.startsWith('ال') && raw.length > 2) {
    const first = raw[2];
    const rest = raw.slice(2);
    const head = 'al-';
    // Optional assimilation preview (simple): if sun letter, you can show doubled consonant
    // but to keep A1-friendly output, we won't double; we just show "al-"
    return head + transliterateToken(rest);
  }

  // Heuristic long vowels: non-initial و → u, non-initial ي → i
  let out = '';
...
function smartTransliterate(arabic: string): string {
  // phrase override wins
  const override = PHRASE_OVERRIDES[arabic.trim()];
  if (override) return override.translit;
...
function addHarakah(arabic: string): string {
  const override = PHRASE_OVERRIDES[arabic.trim()];
  if (override) return override.harakah;
  // per-token light enrichment
  const tokens = arabic.split(/(\s+|[؟!?.,؛،])/);
  return tokens
    .map(tok => {
      if (/^\s+$/.test(tok) || /^[؟!?.,؛،]$/.test(tok)) return tok;
      const base = tok.replace(AR_DIAC, '');
      return TOKEN_OVERRIDES[base]?.harakah ?? tok; // if we know it, inject harakah, else leave as-is
    })
    .join('');
}
...
// -----------------------------------------------------------------------------
// Seeds 1–8 (unchanged from your version)
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// RAW lines (English + Arabic on the same line, separated by a space)
// -----------------------------------------------------------------------------
const RAW_TSV = `
Cheers! في صحتك.
Got it? هل فهمت؟
...
Hello! مرحباً.
`;
// (Keep appending more lines as needed; parser limits to first 200.)

// -----------------------------------------------------------------------------
// Parser (regex splits English part from Arabic part on each line)
// -----------------------------------------------------------------------------
function parseTSV(tsv: string, startId: number): DailyPhrase[] {
  ...
  for (const line of lines) {
    ...
    items.push({
      ...
      words: [
        {
          ...(enrichWord(arabic, english) as Word),
          arabicWithHarakah: withHarakah,
          transliteration: tr,
        },
      ],
      translation: english,
...
const parsedFromDoc = parseTSV(RAW_TSV, basePhrases.length + 1);

// -----------------------------------------------------------------------------
// Final export (safety: ensure fields never empty)
// -----------------------------------------------------------------------------
export const dailyPhrasesDatabase: DailyPhrase[] = [...basePhrases, ...parsedFromDoc].map(p => ({
  ...
  words: p.words.map(w => ({
    ...w,
    arabicWithHarakah: w.arabicWithHarakah?.trim() || w.arabic || '-',
    transliteration: w.transliteration?.trim() || smartTransliterate(w.arabic || ''),
  })),
}));
