// Daily Phrases Database (integrated transliteration + harakah + first 200 lines)
// -----------------------------------------------------------------------------
// Drop-in replacement. No other files need changes.
// - Keeps your interfaces and helpers
// - Preserves your seed items (1–8)
// - Appends first ~200 lines from the uploaded document (embedded as TSV)
// - Auto-fills transliteration and arabicWithHarakah so the UI never shows blanks
...
// -----------------------------------------------------------------------------
// Basic transliteration (lightweight placeholder)
// -----------------------------------------------------------------------------
...
function transliterate(arabic: string): string {
  // greedy replace for ligatures we defined, then char map
  const pre = arabic.replace(/لا/g, 'لا');
  return pre.split('').map(ch => basicMap[ch] ?? ch).join('');
}

function enrichWord(arabic: string, english: string): Word {
  return {
    arabic,
    arabicWithHarakah: arabic, // fallback if diacritics unavailable
    transliteration: transliterate(arabic),
    translation: english,
    grammarType: 'compound', // Use 'compound' for full phrases instead of 'sentence'
...
// -----------------------------------------------------------------------------
// Heuristics for category/difficulty/level (kept lightweight)
// -----------------------------------------------------------------------------
...
return lines.map(line => {
  ...
  // build DailyPhrase for each line
});
  let id = startId;
  return lines.map(line => {
