// Daily Phrases Database — clean final with smart transliteration + sample data
// Drop-in replacement. Preserves your API and helpers.
...
// -----------------------------------------------------------------------------
// Transliteration + Harakah helpers
// -----------------------------------------------------------------------------
const AR_DIAC = /[ًٌٍَُِّْـ]/g; // Arabic diacritics
...
function transliterateToken(token: string): string {
  ...
  // handle "ال" article → al-
  if (raw.startsWith('ال') && raw.length > 2) {
    return 'al-' + transliterateToken(raw.slice(2));
  }
...
// Word builder (avoids TS spreads/any casts)
const makeWord = (
  arabic: string,
  english: string,
...
// -----------------------------------------------------------------------------
// Seeds (1–8) — unchanged from your original
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// RAW sample lines (few only). English + Arabic on each line, separated by a space.
// -----------------------------------------------------------------------------
const RAW_LINES = `
Cheers! في صحتك.
Got it? هل فهمت؟
